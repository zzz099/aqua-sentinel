...
//>>>>>	显示中文字符，包括ASCII码
void 	LCD_SetTextFont(pChineseFont *fonts);								// 设置文本字体，包括中文和ASCII字体
int8_t LCD_EnableChineseFontFromQSPI(void);							// 将中文全字库切换到QSPI内存映射读取
void  GetGB2312Code ( uint8_t * pBuffer, uint16_t code);				// 获取汉字的点阵字库数据
void  LCD_DisplayChinese (uint16_t x,uint16_t y,uint16_t code);	// 显示单个汉字
