...
volatile TouchStructure touchInfo; //	触摸信息结构体，在函数 Touch_Scan()
                                   // 里被调用，存储触摸数据
...
void GT9XX_Reset(void) {
  ...
  HAL_GPIO_WritePin(Touch_RST_PORT, Touch_RST_PIN,
                    GPIO_PIN_RESET); // 拉低复位引脚，此时芯片执行复位
  Touch_IIC_Delay(150000);           // 延时
  HAL_GPIO_WritePin(Touch_RST_PORT, Touch_RST_PIN,
                    GPIO_PIN_SET); // 拉高复位引脚，复位结束
 ...
 *	返 回 值: 无
 *	函数功能: 识别屏幕的版本
 *	说    明: 在 RGB070M1-800*480 V1.1
 * 之前的硬件版本，触摸屏的分辨率为1024*600，
...
void PanelRecognition(void) {
  ...
  if ((HAL_GPIO_ReadPin(Touch_RST_PORT, Touch_RST_PIN) != 1) &&
    ...
    // 在 V1.1
    // 之前的硬件版本，触摸屏的分辨率为1024*600，为了程序上的兼容，这里进行判断处理
...
uint8_t Touch_Init(void) {
  ...
  // 识别屏幕的版本，在 RGB070M1-800*480
  // V1.1以及之后的硬件版本，无需理会此段代码
  // 此段代码只对旧版的7寸屏有用，其它尺寸的屏幕无需理会
  PanelRecognition();
  ...
  if (GT9XX_Info[0] == '9') // 判断第一个字符是否为 9
  {
    ...
    /*---------版本识别，RGB070M1-800*480
     * V1.1以及之后的硬件版本或者其它尺寸的屏幕，无需理会此段代码-----*/

    if (((GT9XX_Info[7] << 8) + GT9XX_Info[6]) ==
        1024) // 判断触摸屏的X轴分辨率是否为1024
    {
      // 在 RGB070M1-800*480 V1.1
      // 之前的硬件版本，触摸屏的分辨率为1024*600，为了程序上的兼容，这里进行判断处理
      //	该变量标志主要用于判断是否需要软件修改采集到的触摸坐标
      Modify_Flag = 1;
    } else if (((GT9XX_Info[7] << 8) + GT9XX_Info[6]) ==
               800) // 触摸屏的X轴分辨率为800
    {
...
void Touch_Scan(void) {
  ...
  if ((touchInfo.num >= 1) && (touchInfo.num <= 5)) //	当触摸数在 1-5 之间时
  {
    for (i = 0; i < touchInfo.num; i++) // 取相应的触摸坐标
    {
      ...
      if (Modify_Flag == 1) {
        touchInfo.y[i] *=
            0.8; // 将1024*600分辨率的触摸坐标换算成800*480的分辨率
        touchInfo.x[i] *=
            0.78; // 将1024*600分辨率的触摸坐标换算成800*480的分辨率
      }
      /*-------------------------------------------------------------------------------------------------*/
    }
